        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if start == -1 or end == -1:
                    ...
                    for i, v in enumerate(tmp):
                        if (
                            flag["location"] < req_end
                            and req_end - 1 > i >= flag["location"]
                        ):
                            field = "required"
                        ...
                        for i, v in enumerate(answer_attrs["required"]):
                            ...
                            for j, v2 in enumerate(v):
                                if v2 == "1":
                                    checks += option[j]
                            output.append(
                                template.format(num=i + 1, check=checks) + "\n"
                            )
...
async def get_answer():
    ...
    start_time = datetime.strptime(
        update["result"]["startTime"], "%Y-%m-%d %H:%M:%S"
    ).date()
    days = (now - start_time).days
    if days < 7:
        end_time = time.strptime(update["result"]["endTime"], "%Y-%m-%d %H:%M:%S")
        end_time = "结束日期: " + time.strftime("%m{m}%d{d}%H{H}%M{M}", end_time).format(
            m="月", d="日", H="时", M="分"
        )
