...
async def wb_del_all(bot: Bot, event: MessageEvent, state: T_State = State()):
    msg = str(event.message).strip()
    if msg:
        state["is_sure"] = msg
...
async def wb_del_all_(bot: Bot, event: MessageEvent, state: T_State = State()):
...
async def wb_del_all_admin_(bot: Bot, event: MessageEvent, state: T_State = State()):
    if state["is_sure"] == "yes":
        res = wb.clean(0)
