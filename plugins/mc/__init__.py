...
mc_server_on = on_command('mcon', aliases={'开服', '开启服务器'},
                          priority=1, rule=to_me())
...
async def mc_server_on_frist(bot: Bot, event: MessageEvent, state: T_State):
    ...
    list_msg = '\n'.join(
        [f'{i}: {line}' for i, line in enumerate(serverID, start=1)])
...
mc_server_off = on_command('mcoff', aliases={'关服', '关闭服务器'},
                           priority=1, rule=to_me())
...
mc_server_restart = on_command('mcrestart', aliases={'重启服', '重启服务器'},
                               priority=1, rule=to_me())
...
async def mc_server_restart_get_server_name(bot: Bot, event: MessageEvent, state: T_State):
    code = state['code']
    await bot.send(event, message=f'请输入验证码: {code}')
