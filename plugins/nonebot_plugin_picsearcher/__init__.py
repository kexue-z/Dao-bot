...
async def get_func(bot: Bot, event: MessageEvent, state: T_State = State()):
    pass


@setu.got("setu", prompt="图呢？")
async def get_setu(bot: Bot, event: MessageEvent, state: T_State = State()):
    """
    发现没有的时候要发问
    :return:
    """
...
@notice_pic.handle()
async def handle_pic(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    try:
...
@previous.handle()
async def handle_previous(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
