...
header = {
    ...
    "Referer": "https://saucenao.com/index.php",
...
async def get_pic_from_url(url: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        data.add_field(
            name="file", value=content, content_type="image/jpeg", filename="blob"
        )
        async with session.post(
            "https://saucenao.com/search.php", data=data, headers=header, proxy=proxy
        ) as res:
            html = await res.text()
